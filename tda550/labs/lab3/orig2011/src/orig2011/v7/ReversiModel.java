...
public class ReversiModel implements GameModel {
	...
	private void tryPlay() {
		if (isPositionEmpty(this.cursorPos)) {
			...
			if (canTurn(this.turn, this.cursorPos)) {
				...
				GameUtils.setGameboardState(board, cursorPos, (this.turn == Turn.BLACK
						? PieceColor.BLACK
						: PieceColor.WHITE));
				
				
				changeSupport.firePropertyChange("nextTurn", this.turn, Turn.nextTurn(this.turn));
				...
				while (x >= 0 && x < size.width && y >= 0 && y < size.height) {
					if (this.board[x][y] == opponentColor) {
						...
						while (!(x == cursorPos.getX() && y == cursorPos.getY())) {
							GameUtils.setGameboardState(board, x, y, myColor);
	...
	public void gameUpdate(final int lastKey) throws GameOverException {
		...
		changeSupport.firePropertyChange("gameUpdate", this.cursorPos, nextCursorPos);
