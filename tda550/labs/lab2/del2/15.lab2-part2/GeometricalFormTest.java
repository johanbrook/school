 ...
 *
 */
public class GeometricalFormTest {
	...
	public static void main(String[] args) {
		...
		try{	
			...
			// These three are equal rectangles:
			...
			// A rectangle isn't a square, even if their sides are equal
			test(rc.equals(sq), "rc.equals(sq)", false);
			test(sq.equals(rc), "sq.equals(rc)", false);
			...
			test(s.compareTo(t) == 0 && s.equals(t), "s.equals(t) && s.compareTo(t)", true);
	...
	private static void compare(int result, String message, int expected){
		...
		String actual = (result < 0) ? "smaller" : (result > 0) ? "larger" : "same";
		
		System.out.println(message + " should be (" + print + "), was ("+ actual +")");
	}
	...
	private static void test(boolean cond, String message, boolean expected){
		...
		System.out.println(message + " should be (" + expected + "), was ("+ cond +")");
	}
